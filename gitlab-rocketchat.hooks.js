...
class Script {
                ...
                default:
                    if (IGNORE_UNKNOWN_EVENTS) {
                        ...
                        return {
                            error: {
                                success: false,
                                message: `unknonwn event ${event}`,
                            },
                        };
                    }
                    result = IGNORE_UNKNOWN_EVENTS
                        ? null
                        : this.unknownEvent(request, event);
                    break;
            }
    ...
    createErrorChatMessage(error) {
        if (IGNORE_ERROR_MESSAGES) {
            return {
                error: {
                    success: false,
                    message: `gitlabevent error: ${error.message}`,
                },
            };
        }
        return {
    ...
    unknownEvent(data, event) {
        const user_avatar = data.user
            ? data.user.avatar_url
            : data.user_avatar || DEFAULT_AVATAR;
        return {
            content: {
                username: data.user
                    ? data.user.name
    ...
    issueEvent(data, event) {
        ...
        const project_avatar =
            project.avatar_url || data.user.avatar_url || DEFAULT_AVATAR;
...
See: ${data.object_attributes.url}`,
    ...
    commentEvent(data) {
        ...
        if (data.merge_request) {
            ...
            text = `commented on commit [${commit.id.slice(0, 8)} ${message}](${
                comment.url
            })`;
        } else if (data.issue) {
    ...
    mergeRequestEvent(data) {
        ...
        const avatar =
            mr.target.avatar_url ||
            mr.source.avatar_url ||
            user.avatar_url ||
            DEFAULT_AVATAR;
        let at = [];
        ...
        if (mr.action === "open" && assignee) {
            ...
            if (
                lastCommitAuthor &&
                lastCommitAuthor.username !== user.username
            ) {
                pushUniq(at, atName(lastCommitAuthor));
            }
    ...
    tagEvent(data) {
        ...
        if (data.checkout_sha === null) {
            ...
            message = `pushed tag [${tag} ${data.checkout_sha.slice(
                0,
                8
            )}](${web_url}/tags/${tag})`;
        }
            ...
            content: {
                ...
                attachments: [
                    makeAttachment(
                        ...
                        `pipeline returned *${
                            pipeline.status
                        }* for commit [${commit.id.slice(0, 8)}](${
                            commit.url
                        }) made by *${commit.author.name}*`,
                        pipeline_time,
